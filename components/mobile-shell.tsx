...
export function MobileShell({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <main className="min-h-dvh overflow-hidden bg-[#080b18] text-zinc-950">
      ...
      <div className="relative z-10 mx-auto flex min-h-dvh w-full max-w-[430px] flex-col bg-[#f7f7fb] pb-24 shadow-2xl shadow-black/40 sm:my-4 sm:min-h-[calc(100dvh-2rem)] sm:overflow-hidden sm:rounded-[2.25rem] sm:border sm:border-white/10 lg:my-0 lg:max-w-[1500px] lg:bg-transparent lg:px-6 lg:py-6 lg:pb-6 lg:shadow-none">
        <div
          className={cn(
            "min-h-dvh sm:min-h-[calc(100dvh-2rem)] lg:min-h-[calc(100dvh-3rem)]",
            className,
          )}
        >
          {children}

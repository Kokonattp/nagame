...
export function getKruakMood({ hasSevereWarning, rainChance, aqi }: MoodInput): Mood {
  if (hasSevereWarning) return { emoji: "😰", tone: "กร๊วกเป็นห่วง…" };
  if (typeof aqi === "number" && aqi > 100) return { emoji: "😷", tone: "ฝุ่นเยอะ พกหน้ากากนะ" };
  if (typeof rainChance === "number" && rainChance >= 60) return { emoji: "🌂", tone: "กร๊วกว่าพกร่มนะ" };
  if (typeof rainChance === "number" && rainChance <= 15) return { emoji: "😎", tone: "วันนี้กร๊วกอารมณ์ดี" };
  return { emoji: "🙂", tone: undefined };
}

...
### `1e72271` — Season radar, แบนเนอร์วันหยุด, ตั๋ว pass, POI อัตโนมัติ
- **การ์ดฤดูกาล 20 เมือง** (ซากุระ/ใบไม้แดง/หิมะ/เทศกาล) ช่วง MM-DD จากค่าเฉลี่ย
  หลายปี คำนวณสถานะเทียบ JST: ป้าย "กำลังพีค" / "อีก X วัน" (≤45 วัน) เรียงใกล้สุดก่อน
- **แบนเนอร์วันหยุดญี่ปุ่น**: Golden Week / Obon / ปีใหม่ โชว์เมื่ออยู่ในช่วงหรือใกล้
  ≤14 วัน — ช่วงปีใหม่คร่อมปี (12-28 → 01-04) ต้องเช็คทั้งกรอบปีนี้และปีก่อน
- **ตั๋ว pass ต่อเมือง** ท้าย transit section ราคาใส่ ~ พร้อมคำเตือนให้เช็คหน้าเคาน์เตอร์
  (ระวังข้อมูลเก่า: ตั๋วบัสรายวัน Kyoto ยกเลิกแล้ว เหลือใบรวมใต้ดิน+บัส ¥1,100)
- **POI อัตโนมัติทุกเมือง**: Wikipedia GeoSearch รัศมี 10 กม. (ฟรี ไม่ใช้ key,
  cache 24 ชม.) filter regex ตัดสถานี/โรงเรียน/โรงแรม/บริษัท เรียงการ์ดมีรูปก่อน —
  ตอบโจทย์ "เพิ่มสถานที่โดยไม่ต้องคัดมือ" ส่วนลิสต์คัดมือยังอยู่หมวดไอเดียทริปตามเดิม

### `99c7c5e` — เตือนภัย JMA, สายรถจากสถานี, mapcode ขับรถ
- **ประกาศเตือนภัย JMA**: bosai JSON รายสำนักงาน — map จังหวัด→รหัสครบ 46 จังหวัด
  + override รายเมือง (Hokkaido แยก 3 เขต, Ishigaki≠Naha, Kagoshima ใช้ 460100)
  แปลไทย ~30 รหัส 3 ระดับ เฝ้าระวัง/เตือนภัย/ขั้นวิกฤต + แบนเนอร์แดงบนสุดเมื่อระดับ
  เตือนภัยขึ้นไป refresh 10 นาที — ต้องลด page revalidate 1800→600 ด้วย
  ไม่งั้น cache ระดับหน้าทับความสดของ warning
- **เลิกใช้คำแปลอากาศของ OpenWeather** ("เมฆเต็มท้องฟ้า") → mapping ไทยเองทั้ง
  WMO code (Open-Meteo) และ condition id (OpenWeather) ใช้ชุดคำเดียวกันสองผู้ให้บริการ
- **Transit คัดมือ 11 เมือง** วาดบน Leaflet ตัวเดิม (จงใจไม่เพิ่ม MapLibre — ไม่เอา
  map engine ซ้อนสองตัว) กดไฮไลต์สาย+ซูม, ลิงก์ Google Maps transit ต่อสาย โดย
  ปลายทาง = ป้ายที่ไกลจากสถานีที่สุด (สายวนป้ายแรก=ป้ายท้าย ใช้ stops[last] ตรง ๆ ไม่ได้)
- **Drive & mapcode 8 เมือง** — หลักการ: ไม่เดาเลข mapcode เด็ดขาด
  - Hokkaido 4 เมือง: ครบจาก PDF ทางการ World Net Rent-A-Car (เป็นรหัสลานจอด)
  - Kawaguchiko: 4 จุด verified จากกระทู้แจก mapcode / Chureito·Oishi ไม่พบแหล่ง
    ที่เชื่อถือได้ → บอกให้ค้น japanmapcode.com เอง
  - Takayama/Izu/Karuizawa: ไม่มีแหล่งเผยแพร่บนเว็บ (Mapple ลงเฉพาะหนังสือ) →
    เบอร์โทร verified สำหรับค้นใน navi แทน + กฎสำคัญในการ์ด: Shirakawa-go จำกัด
    450 คัน/วัน ¥2,000, Kamikochi/Norikura ห้ามรถส่วนตัว (จอด Akandana + shuttle)
- **RSS** ลด cache 30→10 นาที ตามโจทย์ "ทันเหตุการณ์"
- บั๊กแวดล้อมที่เจอ: หยุด task `next start` แล้ว node child ยังถือพอร์ต →
  เซิร์ฟเวอร์เก่าเสิร์ฟโค้ดเก่าเงียบ ๆ ทำผลตรวจเพี้ยน ต้อง kill PID จาก netstat เอง
...
ชุดข้อมูล curated ปัจจุบัน: transit 11 เมือง / drive & mapcode 8 เมือง /
season radar 20 เมือง — เพิ่มเมืองใหม่ = เติม data ใน `lib/cities/*.ts` อย่างเดียว
ส่วน POI อัตโนมัติ (Wikipedia) ครอบทุกเมืองรวมเมืองนอกลิสต์ที่มาจาก geocoding

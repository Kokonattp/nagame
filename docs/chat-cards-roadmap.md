...
⚠ **LINE Login ถูกตัดออกแล้ว (เจ้าของสั่ง 2026-07-17)** — identity ยังเป็น device id + localStorage + cookie
(`lib/game/identity.ts`) ต่อไป

**ผลที่ต้องยอมรับ (ไม่ใช่ปัญหาที่แก้ได้ด้วยโค้ด — เป็น trade-off ที่เลือกแล้ว):**
- ล้าง cache/เปลี่ยนเครื่อง = คนเดิมกลายเป็นคนใหม่ → เมตริก 4 ตัวข้างบน **วัดต่ำกว่าจริงแบบเงียบ**
  (โดยเฉพาะ planning-window retention ที่ต้องจำคนข้าม 14 วัน)
- **ห้าม ship ของสะสมถาวร** (御朱印帳/โปสการ์ดฤดู/Recap ย้อนหลัง) — ของหายเมื่อล้าง cache
  = anti-retention แรงกว่าไม่มีฟีเจอร์ → **Phase 3 พักไว้** จนกว่าจะมี identity ถาวร
- ถ้าวันหน้าอยากได้ retention จริง ต้องกลับมาคุยเรื่อง identity ก่อน (LINE หรือทางอื่น เช่น email/passkey)
...
### Phase 2 — Trip drag-drop + กร๊วกร่างทริป + เมตริก (~2-3 สัปดาห์)
...
**กร๊วกทัก (channel):** in-app welcome-back state เท่านั้น (wishlist×seasons×วันนี้, โค้ดล้วน) — ไม่มี push
เพราะไม่มี LINE OA (ตัดออก 2026-07-17)

**กับดัก (Fable) — hard rules:**
- ⚠️ **~~ห้าม ship ของสะสมถาวรก่อน LINE Login~~ — เจ้าของ override แล้ว 2026-07-17: ใช้ device id ล้วน ยอมรับว่าของหายได้**
  → **นี่คือ trade-off ที่เลือกเอง ไม่ใช่ของที่ลืมคิด — อย่ารื้อกลับโดยไม่ถามเจ้าของ**
  → สิ่งที่ยอมรับ: ล้าง cache / เปลี่ยนเครื่อง / incognito / สลับ browser = **ของสะสมหายเกลี้ยง**
  → ข้อเท็จจริงเชิงเทคนิค: ของอยู่ Supabase อยู่แล้ว (device id mirror ลง cookie) — **ที่หายคือ "กุญแจ" ไม่ใช่ "ของ"**
    ถ้าวันหน้าเปลี่ยนใจ เพิ่มทางกู้ (รหัสกู้คืน 6 หลัก / magic link) ได้โดยไม่ต้องรื้อ schema
  → เขียน UX ให้ซื่อสัตย์: บอกผู้ใช้ตรง ๆ ว่า "ของสะสมผูกกับเครื่องนี้ ล้างข้อมูลเบราว์เซอร์แล้วหาย"
    (ตาม thesis ข้อมูลซื่อสัตย์ — ห้ามให้ผู้ใช้เข้าใจผิดว่าปลอดภัยถาวร)
